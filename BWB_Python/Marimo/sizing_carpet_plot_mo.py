...
def _(mo):
    mo.md("""
    # **Aircraft wing and engine sizing**
    """)
    return
...
@app.cell(hide_code=True)
def _(mo):
    V_cruise_text = mo.ui.text(value="250.7", label=r"Cruise speed, $V_{\text{cruise}}$ [m/s]")
    V_climb_text = mo.ui.text(value="73.56", label=r"Climb speed, $V_{\text{climb}}$ [m/s]")
    V_stall_text = mo.ui.text(value="61.3", label=r"Stall speed, $V_{\text{stall}}$ [m/s]")
    V_takeoff_text = mo.ui.text(value="67.43", label=r"Takeoff speed, $V_{\text{takeoff}}$ [m/s]")
    V_approach_text = mo.ui.text(value="79.7", label=r"Approach speed, $V_{\text{approach}}$ [m/s]")
    h_cruise_text = mo.ui.text(value="12192", label=r"Cruise altitude, $h_{\text{cruise}}$ [m]")
    CD_0_text = mo.ui.text(value="0.0063", label=r"Parasitic drag coeff., $C_{D_0}$")
    CL_max_text = mo.ui.text(value="1.5068", label=r"Max. lift coeff., $C_{L_{\text{max}}}$")
    CL_takeoff_text = mo.ui.text(value="1.245289", label=r"Takeoff lift coeff., $C_{L_{\text{takeoff}}}$")
    n_maneuver_cruise_text = mo.ui.text(value="1.5", label=r"Maneuver load factor (Cruise), $n$")
    n_maneuver_approach_text = mo.ui.text(value="2.5", label=r"Maneuver load factor (Approach), $n$")
    gamma_deg_text = mo.ui.text(value="2.7", label=r"Climb angle, $\gamma$ [deg]")
    e_text = mo.ui.text(value="0.925", label=r"Oswald efficiency factor, $e$")
    AR_text = mo.ui.text(value="4.92347", label=r"Aspect ratio, $AR$")
    ...
    d_landing_text = mo.ui.text(value="1905.", label=r"Landing dist., $d_{\text{landing}}$ [m]")
    d_airborne_text = mo.ui.text(value="300.", label=r"Airborne dist., $d_{\text{airborne}}$ [m]")
    d_takeoff_text = mo.ui.text(value="3352.8", label=r"Takeoff dist., $d_{\text{takeoff}}$ [m]")
    ...
    mo.accordion({r"**Sizing parameters**": mo.vstack([
        ...
        mo.hstack([
            n_maneuver_cruise_text,
            gamma_deg_text,
        ], justify="start"),
        mo.hstack([
            n_maneuver_approach_text,
            gamma_deg_text,
        ], justify="start"),
        mo.hstack([
            e_text,
...
def _(np):
    ...
    def compute_sonic_speed(h):
        ...
        return _a
    return compute_density, compute_sonic_speed


@app.cell
def _(mo):
    max_W_S_text = mo.ui.text(value="4000.", label=r"Maximum wing loading, $W/S$ [N/m^2]")
...
def _(
    ...
    _V_cruise = float(V_cruise_text.value)
    _h_cruise = float(h_cruise_text.value)
    _n_cruise = float(n_maneuver_cruise_text.value)
    _n_approach = float(n_maneuver_approach_text.value)
    ...
    T_W_maneuver_cruise = _CD_0 * _q_cruise / W_S + _n_cruise ** 2 * W_S / np.pi / _e / _AR / _q_cruise
    T_W_maneuver_approach = _CD_0 * _q_approach / W_S + _n_approach ** 2 * W_S / np.pi / _e / _AR / _q_approach
...
def _(
    ...
    # Vertical constraint lines (stall, landing)
    for _W_S, _name in [
        (W_S_stall, "Stall"),
        (W_S_landing, "Landing"),
    ]:
    ...
    # BCR design point — single circle marker at the intersection
    _fig.add_trace(go.Scatter(
        x=[2470.354125],
        y=[0.3946896304],
        mode="markers",
